...
- **Idempotency:** every task this plan wires up is already idempotent by the underlying
  ...
  Bronze upserts via Delta `MERGE` keyed on primary key, or skips re-ingesting an
  already-present `source_file` for the two tables whose source data can legitimately
  contain same-key duplicates (`order_items`, `events` — see `pipelines/bronze/README.md`);
  Silver/Gold overwrite; feature tables delete-then-insert by snapshot/window). A retried
  task re-running produces the same output, not duplicates — nothing new to build here.
  (This wasn't always true — Bronze's original blind `mode="append"` re-duplicated every row
  on every re-run; fixed via `DeltaWriter.merge()`/`append_if_new_source`, found live while
  verifying `build_gold.py`'s new FK constraints against real data.)
...
**Correction to step 4, from hands-on verification**: manually triggering all three DAGs
"now" does **not** actually satisfy `dp2_gold`/`dp3_feature`'s `ExternalTaskSensor`s. The
sensor's `execution_delta` requires an *exact* logical-date match against the upstream DAG's
run — three independent "trigger now" calls land at three different timestamps a few seconds
apart, which doesn't equal the required 1h / 1h30m offset. What actually works for a manual
smoke test: either (a) trigger `dp1_bronze` alone and let the *scheduler's own* auto-fired
runs for `dp2_gold`/`dp3_feature` pick it up on their normal schedule (they align correctly —
verified), or (b) trigger each DAG with an explicit `-e <timestamp>` computed to match the
required offset, keeping the target timestamp in the past relative to wall-clock time
(Airflow won't start a DagRun until its logical_date has passed, even for manual triggers).

### Status — DP1/DP2/DP3 verified working, screenshot still the only gap

All three DAGs have been run successfully end-to-end against local docker-compose
(MinIO/Postgres) — confirmed via CLI/API inspection (`airflow tasks states-for-dag-run`,
task logs), including `dp1_bronze`'s `ingest_bronze`/`validate_bronze` tasks both reaching
`success`. **What's still missing for this rubric section's full point value is the
literal Airflow UI screenshot** — per §1's table, "showing the stages and their order" is the
literal ask, which the **DAG Graph view** (not the DAGs-list view `assets/draft-airflow.png`
currently shows — that one displays all three DAGs paused with zero runs, which doesn't
satisfy this) would satisfy even without a live run; a Grid view showing a green run is
stronger evidence but not the minimum bar.
