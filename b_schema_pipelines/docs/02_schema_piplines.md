...
* **Incremental:** Bronze upserts via Delta `MERGE`, keyed on each table's primary key (`customers`/`products`/`orders`/`payments`), or skips re-ingesting a `source_file` already present in Bronze for tables whose source can legitimately contain same-key duplicates (`order_items`' Problem C, `events`' Problem F — see `pipelines/bronze/README.md`'s "Idempotency" section for why those two can't use a keyed `MERGE`). Feature tables use incremental snapshot per `event_timestamp`.
...
* **Backfill Policy:** Re-run Bronze → Silver → Gold → Features in sequence for the target date range. Bronze's `MERGE`/skip-if-ingested idempotency (not `pipeline_run_id`, which is unique per run and was never actually usable as a dedup key — a real bug, found and fixed) means a backfill re-run against unchanged source files is a no-op, not a duplicate.
