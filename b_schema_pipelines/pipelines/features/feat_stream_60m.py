...
class StreamFeature60m(PipelineBase):
    ...
    def _ensure_index(self) -> None:
        ...
        join key feat_customer_unified.py's as-of join filters/joins on.
        `IF NOT EXISTS` makes this a cheap no-op after the first successful
        run; called after the write so the table is guaranteed to exist."""
        with closing(psycopg2.connect(
            host=self.postgres_host,
            port=self.postgres_port,
            dbname=self.postgres_db,
